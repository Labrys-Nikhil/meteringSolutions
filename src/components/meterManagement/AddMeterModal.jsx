...
// components/meterManagement/AddMeterModal.jsx
// import React from 'react';
// import { Search, User, Plus, Trash2, X } from 'lucide-react';
...
//                       selectedUser === user.id ? 'bg-blue-50 border-blue-200' : 'hover:bg-gray-50'
//                     }`}
//                   >
//                     <div className="flex items-center">
//                       <div className="bg-gray-200 border-2 border-dashed rounded-xl w-10 h-10 flex items-center justify-center mr-3">
...
// import { Search, User, Plus, Trash2, X, CheckCircle, Zap } from 'lucide-react';
...
//     if (selectedMeterId.includes(meterId)) {
//       setSelectedMeterIds(selectedMeterId.filter(id => id !== meterId));
//     } else {
//       setSelectedMeterIds([...selectedMeterId, meterId]);
//     }
//   };
...
//             {/* Two Column Layout for User and Meter Selection */}
...
//               {/* Unassigned Meters Column */}
//               <div>
//                 <label className="block text-sm font-medium text-gray-700 mb-2">
//                   Select Unassigned Meters
//                   {selectedMeterId.length > 0 && (
//                     <span className="ml-2 text-sm text-blue-600">
//                       ({selectedMeterId.length} selected)
//                     </span>
//                   )}
//                 </label>
//                 <div className="relative">
...
//                 <div className="mt-2 max-h-60 overflow-y-auto border border-gray-200 rounded-md">
//                   {unasignedIOTMeter.map(meter => (
//                     <div
//                       key={meter.id}
//                       onClick={() => handleMeterSelection(meter.deviceId)}
//                       className={`p-3 border-b border-gray-100 cursor-pointer transition-colors ${
//                         selectedMeterId.includes(meter.meterId)
...
//                               RS485 ID: {meter.meterSerialNumber}
//                             </div>
...
//                         {selectedMeterId.includes(meter.id) && (
...
//                     </div>
//                   ))}
//                   {unasignedIOTMeter.length === 0 && (
//                     <div className="p-4 text-center text-gray-500">
//                       {unasignedIOTMeter.length === 0 ? 'No unassigned meters available' : 'No meters match your search'}
//                     </div>
//                   )}
//                 </div>
//               </div>
...
//             <button
//               onClick={handleAddNewMeters}
//               disabled={!selectedUser && selectedMeterId.length === 0 && newMeters.every(m => !m.meterId)}
//               className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//             >
//               {selectedMeterId.length > 0 ? `Assign ${selectedMeterId.length} Meter(s)` : 'Add Meters'}
//             </button>
//           </div>
//         </div>
//       </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-blue-100/30 backdrop-blur-sm">
      <div className="bg-white/90 backdrop-blur-lg rounded-xl shadow-2xl w-full max-w-4xl max-h-screen">
        <div className="p-6 max-h-[80vh] flex flex-col overflow-hidden">
          ...
          {/* Modal Header */}
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-900">Assign Meter</h2>
            <button
              onClick={() => setShowAddMeterModal(false)}
              className="text-gray-500 hover:text-gray-700"
            >
          ...
          <div className="space-y-6 flex-1 overflow-y-auto p-1">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Meter Selection */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Select a Meter</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                  <input
                    type="text"
