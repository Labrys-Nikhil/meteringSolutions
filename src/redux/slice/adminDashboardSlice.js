...
const adminDashboardDataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getTemperedMeter.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getTemperedMeter.fulfilled, (state, action) => {
        state.temperedMeters = action.payload; // ✅ save data
        state.loading = false;
      })
      .addCase(getTemperedMeter.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || action.error.message;
      })

      .addCase(fetchOfflineOnlineFaultyMeter.pending, (state) => {
                state.loading = true;
                state.error = null;
            })
            .addCase(fetchOfflineOnlineFaultyMeter.fulfilled, (state, action) => {
                state.data = action.payload;
                state.loading = false;
            })
            .addCase(fetchOfflineOnlineFaultyMeter.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload || action.error.message;
            })
  },
...
export const selectDailyConsuption = (state) =>
  state.adminDashboard.dailyConsumption;
export const selectAdminUserMeterData = (state) =>
  state.adminDashboard.adminUserMeterData;
export const selectFetchDashboardData = (state) => state.adminDashboard.data;
export const selectTemperedMeters = (state) => state.adminDashboard.temperedMeters;
