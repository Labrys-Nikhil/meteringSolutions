...
//                       <div className="body-sm   font-medium text-gray-900">{meter.name}</div>
...
//                           <p className="body-xs  text-blue-700">DevEUI: {selectedMeter.devEUI}</p>
//                           <p className="body-xs  text-blue-700">RS485 ID: {selectedMeter.rs485Id}</p>
//                         </div>
//                       </div>
//                     </div>
...
//                     <div>
//                       <label className="block body-xs  font-medium text-gray-700 mb-2">Command Type</label>
//                       <select
//                         value={commandType}
//                         onChange={(e) => {
...
//                       </select>
//                       {commandType && (
//                         <p className="mt-2 body-xs  text-gray-600">{commandTemplates[commandType].description}</p>
//                       )}
//                     </div>
...
//                           <div key={param.name}>
//                             <label className="block body-xs  font-medium text-gray-700 mb-1 capitalize">
...
//                     {commandType === 'custom' && (
//                       <div>
//                         <label className="block body-xs  font-medium text-gray-700 mb-2">Custom Hex Payload</label>
//                         <input
//                           type="text"
//                           value={customPayload}
...
//                       <div>
//                         <label className="block body-xs  font-medium text-gray-700 mb-2">Port</label>
//                         <input
//                           type="number"
//                           min="1"
//                           max="255"
...
//                         />
//                         <label htmlFor="confirmed" className="ml-2 body-xs  text-gray-700">
...
//                         <div className="font-mono body-xs  bg-white p-3 rounded border">
...
//                         <div className="grid grid-cols-1 md:grid-cols-2 gap-2 body-xs  text-gray-600">
...
import { useDispatch, useSelector } from "react-redux";
import {
  addActionHistoryToMeter,
  selectIotMeters,
  selectMeteringMeters,
} from "../redux/slice/meterManagementSlice";
import {
  fetchMeters,
  fetchUnassignedIoTMeters,
} from "../redux/thunks/meterThunks";
import { sendDownlinlkCommand } from "../redux/thunks/meterManagementThunks";
import { userAllData, userQueryData } from "../redux/slice/userMangementSlice";
import { toast } from "react-toastify";
import { fetchUsersByQuery } from "../redux/thunks/userManagementThunks";
...
const MeterManagement = () => {
  ...
  const commandTemplates = {
    ...
    forced_eb_dg_reset: {
      name: "Forced EB_DG Rst",
      description: "Reset forced EB/DG state",
      payload: "",
      params: [],
    },
    balance_deduct_random: {
      name: "Balance Deduct Random",
      description: "Deduct random balance",
      payload: "",
      params: [
        {
    ...
    set_overload_attempt_wait_time: {
      name: "Set Overload Attempt Wait Time",
      description: "Set wait time (in seconds) after overload",
      payload: "AA060028{time}",
      params: [
        {
  ...
  // Filter meters based on search term
  const filteredMeters = meteringMeter.filter(
    (meter) =>
      (meter?.name?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
      (meter?.id?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
      (meter?.devEUI?.toLowerCase() || "").includes(searchTerm.toLowerCase())
  ...
  const handleSendCommand = async () => {
    ...
    const getParamValue = () => {
      if (commandTemplates[commandType].params.length > 0) {
        const firstParam = commandTemplates[commandType].params[0];
        return selectedParams[firstParam.name] ?? firstParam.default ?? "";
      }
      return "";
    };
    ...
    try {
      ...
      const result = await dispatch(
        sendDownlinlkCommand({ payload, port })
      ).unwrap();
  ...
  console.log("all meters data", {
    iot: unasignedIOTMeter,
    metering: meteringMeter,
  });
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
                ...
                {selectedMeter ? (
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      {commandType && (
                        <p className="mt-2 body-xs text-gray-600">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center">
                        ...
                        <label
                          htmlFor="confirmed"
                          className="ml-2 body-xs  text-gray-700"
                        >
                          Require confirmation (ACK)
                        </label>
                      </div>
